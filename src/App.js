...
// Build artist->genre lookup from concert data (replaces hardcoded GENRE_MAP)
...
// Get genre color for a concert (may have multiple bands)
function getConcertGenreInfo(concert, genreMap) {
  const bands = Array.isArray(concert.bands) ? concert.bands : [];
  // Use concert-level genre first
  const concertGenre = concert.genre;
  if (concertGenre) {
    return { genre: concertGenre, color: GENRE_COLORS[concertGenre] || GENRE_COLORS['Other'], mixed: false };
  }
  // Derive from bands
  const genres = [...new Set(bands.map(b => genreMap[b]).filter(Boolean))];
  if (genres.length === 0) return { genre: null, color: GENRE_COLORS['Other'], mixed: false };
...
// Genre badge component — rainbow for mixed, solid for single
const GenreBadge = ({ genre, color, mixed, small = false }) => {
  ...
  if (mixed) {
    return (
      <span style={{
        ...sz, display: 'inline-block', borderRadius: 3, fontFamily: "'Space Mono', monospace",
        letterSpacing: '0.08em', textTransform: 'uppercase', fontWeight: 700, color: '#fff',
        background: 'linear-gradient(90deg,#00f2ff,#9d00ff,#ff00ff,#ff0077,#ffcc00,#ffaa00)',
        border: 'none',
      }}>MIXED</span>
    );
  }
  ...
  return (
    <span style={{
      ...sz, display: 'inline-block', borderRadius: 3, fontFamily: "'Space Mono', monospace",
      letterSpacing: '0.08em', textTransform: 'uppercase', color: col,
      background: hexToRgba(col, 0.12), border: `1px solid ${hexToRgba(col, 0.4)}`,
    }}>{genre}</span>
  );
};
...
const MarqueeStyles = () => (
  <style>{`
    ...
    @keyframes rainbow-border {
      0%   { border-color: #00f2ff; box-shadow: 0 0 12px #00f2ff44; }
      25%  { border-color: #9d00ff; box-shadow: 0 0 12px #9d00ff44; }
      50%  { border-color: #ff00ff; box-shadow: 0 0 12px #ff00ff44; }
      75%  { border-color: #ffcc00; box-shadow: 0 0 12px #ffcc0044; }
      100% { border-color: #00f2ff; box-shadow: 0 0 12px #00f2ff44; }
    }
...
const Card = ({ children, style = {}, glow = false, neon = false, genreColor = null, onClick }) => {
  ...
  const borderColor = genreColor || (glow ? C.teal : neon ? nb.border : C.border);
  const glowColor   = genreColor ? hexToRgba(genreColor, 0.2) : glow ? C.tealGlow : neon ? nb?.glow : null;
  return (
    <div onClick={onClick} style={{
      background: genreColor ? `linear-gradient(135deg, ${C.bgCard}, ${hexToRgba(genreColor, 0.07)})` : C.bgCard,
      border: `1px solid ${borderColor}`, borderRadius: 8, padding: 16,
      cursor: onClick ? 'pointer' : 'default',
      boxShadow: glowColor ? `0 0 16px ${glowColor}, 0 2px 8px rgba(0,0,0,0.4)` : '0 2px 8px rgba(0,0,0,0.4)',
      ...style,
    }}>{children}</div>
  );
};
...
const Btn = ({ children, onClick, variant = 'primary', style = {}, disabled = false }) => {
  ...
  return (
    <button onClick={onClick} disabled={disabled} style={{ fontFamily:"'Space Mono', monospace", fontSize:10, letterSpacing:'0.12em', textTransform:'uppercase', border:'none', borderRadius:4, padding:'8px 16px', cursor:disabled?'not-allowed':'pointer', opacity:disabled?0.5:1, transition:'all 0.15s', ...variants[variant], ...style }}>{children}</button>
  );
};
...
function UpcomingModal({ show, onClose, onSave, onDelete }) {
  ...
  const handleSave = async () => {
    if (!form.artist || !form.date) return alert('Artist and date are required.');
    setSaving(true);
...
function EditModal({ concert, onClose, onSave, onDelete }) {
  ...
  const handleSave = async () => {
    ...
    await onSave(concert?.id, { ...form, bands: bandList, has_setlist: !!(form.has_setlist_names?.trim()) });
    setSaving(false);
  };
  ...
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.85)', zIndex:1000, display:'flex', alignItems:'center', justifyContent:'center', padding:20, backdropFilter:'blur(4px)' }} onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="fade-in" style={{ background:C.bgCard, border:`1px solid ${C.teal}`, borderRadius:10, padding:24, width:'100%', maxWidth:520, maxHeight:'90vh', overflowY:'auto', boxShadow:`0 0 40px ${C.tealGlow}` }}>
        ...
        <div style={fld}><label style={lbl}>State</label><input style={{ ...inp, width:80 }} value={form.state} onChange={e => set('state', e.target.value)} maxLength={2} /></div>
        <div style={fld}>
        ...
        <div style={fld}><label style={lbl}>Setlists Obtained (comma separated band names)</label><input style={inp} value={form.has_setlist_names} onChange={e => set('has_setlist_names', e.target.value)} /></div>
...
function OnThisDay({ concerts }) {
  ...
  return (
    <div style={{ background:`linear-gradient(135deg,${C.bgCard},${C.bgCardAlt})`, border:`1px solid ${C.teal}`, borderRadius:8, padding:'14px 18px', marginBottom:16, animation:'pulse-teal 3s ease-in-out infinite' }}>
      <div style={{ fontFamily:"'Space Mono', monospace", fontSize:9, letterSpacing:'0.25em', textTransform:'uppercase', color:C.teal, marginBottom:10 }}>📅 On This Day — {today.toLocaleDateString('en-US',{month:'long',day:'numeric'})}</div>
      {matches.map((ev, idx) => (
        <div key={ev.id} style={{ display:'flex', alignItems:'flex-start', gap:12, marginBottom:8, paddingBottom:8, borderBottom:idx<matches.length-1?`1px solid ${C.border}`:'none' }}>
          ...
          <div>
            ...
            <div style={{ fontFamily:"'Space Mono', monospace", fontSize:8, color:C.gray, textTransform:'uppercase', letterSpacing:'0.08em' }}>{[ev.venue,ev.city,ev.state].filter(Boolean).join(' · ')}</div>
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <Card neon style={{ minHeight:150, display:'flex', flexDirection:'column', justifyContent:'center' }}>
      ...
      <div className="fade-in" key={index}>
        ...
        <div style={{ fontSize:'0.8rem', color:C.gray, lineHeight:1.4 }}>{active.sub}</div>
      </div>
...
function RandomShow({ concerts }) {
  ...
  const spin = () => {
    ...
    setSpinning(true);
    let i = 0;
    const iv = setInterval(()=>{ setShow(concerts[Math.floor(Math.random()*concerts.length)]); if(++i>=12){clearInterval(iv);setSpinning(false);} },70);
  };
...
function DonutChart({ fest, solo }) {
  const total = fest+solo||1, festPct=fest/total;
        ...
        {[[C.teal,'Festival Days',fest],[C.grayDim,'Standalone',solo]].map(([color,label,val])=>(
          <div key={label} style={{ marginBottom:10 }}>
            <div style={{ display:'flex', justifyContent:'space-between', marginBottom:3 }}>
              <span style={{ fontFamily:"'Space Mono', monospace", fontSize:8, color:C.gray, textTransform:'uppercase' }}>{label}</span>
...
function TopFestBlocks({ festBreakdown }) {
  ...
  const colors=[C.teal,C.cyan,C.purple], medals=['🥇','🥈','🥉'];
  return (
      ...
      {top3.map(([name,count],i)=>(
        <div key={name} style={{ display:'flex', alignItems:'center', gap:12, background:`${colors[i]}0a`, border:`1px solid ${colors[i]}33`, borderLeft:`3px solid ${colors[i]}`, borderRadius:4, padding:'10px 14px' }}>
          ...
          <div style={{ textAlign:'right' }}>
            ...
            <div style={{ fontFamily:"'Space Mono', monospace", fontSize:7, color:C.gray }}>days</div>
          </div>
        </div>
      ))}
      ...
      {Object.entries(dec).map(([decade,count],i)=>(
        <div key={decade} style={{ background:`${colors[i]}18`, border:`1px solid ${colors[i]}44`, borderBottom:`3px solid ${colors[i]}`, borderRadius:4, padding:'10px 8px', textAlign:'center' }}>
          ...
          <div style={{ marginTop:6, height:2, background:C.border, borderRadius:1, overflow:'hidden' }}>
            <div style={{ height:'100%', width:`${(count/max)*100}%`, background:colors[i] }} />
          </div>
        </div>
      ))}
    </div>
...
function HallOfFame({ sets, genreMap, onShare }) {
  ...
  const artists = useMemo(() => {
    const m = {};
    sets.forEach(s => {
      if (!m[s.artist]) m[s.artist] = { artist:s.artist, shows:[], genre:null };
      m[s.artist].shows.push(s);
      if (s.genre && !m[s.artist].genre) m[s.artist].genre = s.genre;
    });
    // also pull from genreMap
          ...
          return (
            <div key={a.artist} onClick={()=>handleSelect(a.artist,isSelected)}
              ...
              style={{
                ...
                borderRadius:8, padding:'12px 14px', cursor:'pointer', transition:'all 0.18s', position:'relative',
              }}>
...
function DayCard({ event, onEdit, genreMap={} }) {
  ...
  return (
    <div className="day-card-hover" data-date={event.date}
      style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderLeft:`3px solid ${borderColor}`, borderRadius:8, marginBottom:8, overflow:'hidden' }}
      ...
      <div style={{ padding:'8px 16px 12px', display:'flex', flexWrap:'wrap', gap:5 }}>
        {bands.map((b,i)=>{
          ...
          return (
            <span key={i} style={{ fontSize:'0.78rem', color:C.white, background:bg?hexToRgba(bg,0.15):C.bgCardAlt, border:`1px solid ${bg?hexToRgba(bg,0.4):C.border}`, borderRadius:4, padding:'4px 9px' }}>{b}</span>
          );
        })}
      </div>
    </div>
...
function TimelineTab({ concerts, setActiveTab, genreMap }) {
  const yearsData = useMemo(()=>{
    ...
    return Object.entries(groups).sort((a,b)=>b[0]-a[0]).map(([year,yearShows])=>{
      const finalFlow=[], usedMarkers=new Set();
      let showCounter=0;
  ...
  return (
    <div style={{ padding:'24px 0' }} className="fade-in">
      {dayGroups.map(event=>event.is_festival
        ? <FestivalScheduleCard key={event.id} event={event} genreMap={genreMap} />
        : <DayCard key={event.id} event={event} onEdit={onEdit} genreMap={genreMap} />
      )}
...
// ─── BY FESTIVAL TAB (big hero headers) ──────────────────────────────────────
  ...
  return (
    <div style={{ marginTop:20 }} className="fade-in">
      {festGroupings.map((fest, fi) => {
        ...
        const firstYear = yearsSorted[yearsSorted.length-1];
        const lastYear  = yearsSorted[0];
        ...
        return (
          <div key={fest.name} style={{ marginBottom:48 }}>
            ...
            <div style={{ position:'relative', borderRadius:12, overflow:'hidden', marginBottom:20, background:`linear-gradient(135deg, ${hexToRgba(color,0.12)}, ${C.bgCard})`, border:`1px solid ${hexToRgba(color,0.5)}`, boxShadow:`0 0 40px ${hexToRgba(color,0.2)}, 0 4px 20px rgba(0,0,0,0.6)`, padding:'32px 36px' }}>
              ...
              <div style={{ position:'absolute', bottom:-40, left:20, width:200, height:200, background:`radial-gradient(circle, ${hexToRgba(color,0.08)}, transparent)`, pointerEvents:'none' }} />
              ...
              <div style={{ position:'relative', zIndex:1 }}>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:24, marginBottom:20 }}>
                  {[
                    ...
                    [yearsSorted.length, yearsSorted.length===1?'YEAR':'YEARS'],
                    [uniqueArtists, 'UNIQUE ARTISTS'],
                    [yearRange, 'SPAN'],
                  ].map(([val,label])=>(
                    <div key={label}>
                      <div style={{ fontFamily:"'Bebas Neue'", fontSize:'2rem', color, lineHeight:1 }}>{val}</div>
                      <div style={{ fontFamily:"'Space Mono', monospace", fontSize:8, color:C.gray, textTransform:'uppercase', letterSpacing:'0.12em', marginTop:2 }}>{label}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display:'flex', flexWrap:'wrap', gap:6 }}>
                  {yearsSorted.map(yr=>(
                    <span key={yr} style={{ fontFamily:"'Space Mono', monospace", fontSize:9, background:hexToRgba(color,0.18), color, border:`1px solid ${hexToRgba(color,0.45)}`, padding:'3px 10px', borderRadius:4, cursor:'pointer' }}
                      onClick={()=>toggle(fest.name,yr)}>
                      {yr} {collapsed[`${fest.name}-${yr}`] ? '▸' : '▾'}
                    </span>
                  ))}
                </div>
              </div>
              ...
              return (
                <div key={yr} style={{ marginBottom:16 }}>
                  ...
                  {!isCollapsed && shows.sort((a,b)=>a.date.localeCompare(b.date)).map(s=>(
                    <FestivalScheduleCard key={s.id} event={s} compact />
                  ))}
                </div>
              );
            })}
          ...
          <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden' }}>
            <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.78rem' }}>
              ...
              <tbody>
                ...
                {!paged.length && <tr><td colSpan={6} style={{ textAlign:'center', color:C.gray, padding:40 }}>No results.</td></tr>}
              </tbody>
            </table>
          </div>
          {totalPages>1 && (
              ...
              <div key={row.artist} style={{ background:gc?`linear-gradient(135deg,${C.bgCard},${hexToRgba(gc,0.1)})`:C.bgCard, border:`1px solid ${gc?hexToRgba(gc,0.5):C.border}`, boxShadow:gc?`0 0 12px ${hexToRgba(gc,0.2)}`:'none', borderRadius:8, padding:'14px 16px', position:'relative' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:8, marginTop:10, paddingTop:10, borderTop:`1px solid ${C.border}` }}>
                  ...
                  <select
                    ...
                    style={{ flex:1, background:gc?hexToRgba(gc,0.15):C.bgCardAlt, border:`1px solid ${gc?hexToRgba(gc,0.4):C.border}`, borderRadius:4, color:gc||C.gray, fontSize:9, padding:'3px 6px', fontFamily:"'Space Mono'", cursor:'pointer' }}
                  >
                    <option value="">— unset —</option>
                    {GENRES.map(g=><option key={g} value={g}>{g}</option>)}
                  </select>
    ...
    <div style={{ padding:'24px 0' }} className="fade-in">
      ...
      <div style={{ background:C.bgCard, border:`1px solid ${C.border}`, borderRadius:8, overflow:'hidden' }}>
        <table style={{ width:'100%', borderCollapse:'collapse', fontSize:'0.78rem' }}>
          <thead>
            <tr style={{ background:C.bgCardAlt }}>
              {['Date','Artists','Venue','City','Genre','Type','Setlist','Edit'].map(h=>(
...
export default function App() {
  ...
  const [upcomingModal, setUpcomingModal] = useState(null); // null | 'new' | show object
  ...
  // Artist->genre lookup built from concert data (no hardcoded map)
  ...
  const applyFilters = useCallback((list, isSet=false) => {
    ...
    if (search) {
      const q=search.toLowerCase();
      d=d.filter(r=>{ const bands=isSet?[r.artist]:(r.bands||[]); return bands.some(b=>b.toLowerCase().includes(q))||(r.venue||'').toLowerCase().includes(q)||(r.city||'').toLowerCase().includes(q)||(r.festival_name||'').toLowerCase().includes(q)||(r.festival_day||'').toLowerCase().includes(q); });
    }
    return d;
  }, [yearFilter, festFilter, search]);
  ...
  async function handleDelete(id) {
    if (window.confirm('Delete show?')) { await supabase.from('concerts').delete().eq('id', id); fetchConcerts(); setEditTarget(null); }
  }

  // Set genre on ALL concerts for an artist at once
  ...
  async function handleUpcomingSave(id, payload) {
    ...
    fetchUpcoming(); setUpcomingModal(null);
  }
  async function handleUpcomingDelete(id) {
    if (window.confirm('Delete?')) { await supabase.from('upcoming_concerts').delete().eq('id', id); fetchUpcoming(); setUpcomingModal(null); }
  }
  ...
  return (
    <div style={{ background:C.bg, minHeight:'100vh', paddingBottom:60 }}>
      ...
      {/* ── STATS HEADER BAR ── */}
      <div style={{ background:`linear-gradient(180deg,#050508 0%,${C.bgCard} 100%)`, borderBottom:`1px solid ${C.teal}22`, padding:'20px 24px 0' }}>
        <div style={{ maxWidth:1200, margin:'0 auto' }}>
          {/* Big stat bar */}
          <div style={{ display:'grid', gridTemplateColumns:'repeat(4,1fr)', borderBottom:`1px solid ${C.border}`, marginBottom:0 }}>
            {[
              [headerStats.totalSets.toLocaleString(), 'TOTAL SETS', 'individual performances'],
              [headerStats.uniqueArtists, 'UNIQUE ARTISTS', 'bands & performers'],
              [headerStats.totalShows, 'SHOW DAYS', `${headerStats.festDays} fest · ${headerStats.totalShows-headerStats.festDays} solo`],
              [headerStats.setlistCount, 'SETLISTS', 'physical collections 📋'],
            ].map(([val,label,sub])=>(
              ...
              <div style={{ background:'#0a0a0a', border:'3px solid #222', borderRadius:12, overflow:'hidden', boxShadow:'0 0 30px rgba(255,204,0,0.15)' }}>
                ...
                <div style={{ padding:16 }}>
                  ...
                  <div style={{ maxHeight:180, overflowY:'auto' }}>
                    {upcoming.map((show,i)=>(
                      <div key={show.id||i} style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:10, paddingBottom:10, borderBottom:'1px solid #1a1a1a' }}>
                        <div style={{ flex:1 }}>
                          <div className="marquee-letter" style={{ fontSize:'1.1rem', lineHeight:1 }}>{show.artist}</div>
                          <div style={{ color:'#666', fontSize:8 }}>@{show.venue||'TBA'} · {show.date}</div>
                        </div>
                        <div style={{ display:'flex', alignItems:'center', gap:8, flexShrink:0 }}>
                          <span style={{ fontFamily:"'Space Mono'", fontSize:7, color:'#ffcc00', background:'rgba(255,204,0,0.15)', border:'1px solid rgba(255,204,0,0.3)', padding:'2px 6px', borderRadius:3 }}>{show.status||'TICKETS BOUGHT'}</span>
                          <button onClick={()=>setUpcomingModal(show)} style={{ background:'none', border:'1px solid #333', color:'#999', cursor:'pointer', fontSize:9, borderRadius:3, padding:'2px 6px', fontFamily:"'Space Mono'" }}>EDIT</button>
                        </div>
                      </div>
                    ))}
