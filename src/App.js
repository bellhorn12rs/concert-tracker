...
// ─── UTILITY ──────────────────────────────────────────────────────────────────
// Defined at file top so every component below can use it without hoisting issues
...
const MarqueeStyles = () => (
  <style>{`
    ...
    @keyframes pulse-teal {
      0%, 100% { box-shadow: 0 0 24px rgba(0,229,204,0.15); }
      50%       { box-shadow: 0 0 40px rgba(0,229,204,0.35); }
    }
...
const Badge = ({ children, color = C.teal, bg = C.tealFaint }) => (
  <span style={{
    ...
    color, background: bg, border: `1px solid ${color}44`, padding: '2px 6px', borderRadius: 3,
  }}>{children}</span>
);
...
const Card = ({ children, style = {}, glow = false, neon = false, onClick }) => {
...
const CardTitle = ({ children, style = {} }) => (
  <div style={{
    ...
    marginBottom: 12, paddingBottom: 8, borderBottom: `1px solid ${C.border}`, ...style,
  }}>{children}</div>
);
    ...
    >
      <div className="fade-in" style={{ background: C.bgCard, border: `1px solid ${C.teal}`, borderRadius: 10, padding: 24, width: '100%', maxWidth: 520, maxHeight: '90vh', overflowY: 'auto', boxShadow: `0 0 40px ${C.tealGlow}` }}>
        ...
        <div style={fld}>
          ...
          <input style={inp} value={form.has_setlist_names || ''} onChange={e => set('has_setlist_names', e.target.value)} placeholder="e.g. Ween, Guster" />
        </div>
  ...
  return (
    ...
    >
      <div className="fade-in" style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ background: `linear-gradient(135deg, ${C.bg}, ${C.bgCard}, ${C.bgCardAlt})`, border: `1px solid ${C.teal}`, borderRadius: 12, padding: '28px 24px', boxShadow: `0 0 40px ${C.tealGlow}`, position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ fontFamily: "'Bebas Neue'", fontSize: 'clamp(1.8rem,6vw,2.6rem)', letterSpacing: '0.06em', color: C.white, lineHeight: 1, marginBottom: 16 }}>{artist}</div>
          ...
          <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 7, color: C.grayDim, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
        ...
        <div style={{ display: 'flex', gap: 8, marginTop: 12, justifyContent: 'center' }}>
          ...
          <button onClick={onClose} style={{ fontFamily: "'Space Mono', monospace", fontSize: 9, letterSpacing: '0.1em', textTransform: 'uppercase', background: C.bgCard, color: C.gray, border: `1px solid ${C.border}`, borderRadius: 4, padding: '8px 16px', cursor: 'pointer' }}>Close</button>
        </div>
      </div>
    </div>
  );
...
function OnThisDay({ concerts }) {
  ...
  return (
    <div style={{ background: `linear-gradient(135deg, ${C.bgCard}, ${C.bgCardAlt})`, border: `1px solid ${C.teal}`, borderRadius: 8, padding: '14px 18px', marginTop: 20, marginBottom: 8, animation: 'pulse-teal 3s ease-in-out infinite' }}>
...
// ─── ARTIST INSIGHTS (cycling widget) ─────────────────────────────────────────
function ArtistInsights({ concerts }) {
  ...
  const insights = useMemo(() => {
    ...
    return [
      { label: 'PEAK INTENSITY', val: peakYear?.[0],           sub: `Your busiest year with ${peakYear?.[1]} shows logged.` },
      { label: 'HOME TURF',      val: topCity?.[0]?.toUpperCase(), sub: `You've caught ${topCity?.[1]} shows here.` },
      { label: 'FESTIVAL RATIO', val: `${festPct}%`,           sub: `${festPct}% of your history happened in a field.` },
      { label: 'TOTAL LEGACY',   val: concerts.length,          sub: `Unique show days since your journey began.` },
    ];
  }, [concerts]);

  useEffect(() => {
    if (!insights.length) return;
    const t = setInterval(() => setIndex(p => (p + 1) % insights.length), 6000);
    return () => clearInterval(t);
  }, [insights.length]);
  ...
  return (
    <Card neon style={{ minHeight: 150, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
      ...
      <div className="fade-in" key={index}>
        <div style={{ fontFamily: "'Bebas Neue'", fontSize: '2.4rem', color: C.white, lineHeight: 1, marginBottom: 4 }}>{active.val}</div>
        <div style={{ fontSize: '0.8rem', color: C.gray, lineHeight: 1.4, maxWidth: '90%' }}>{active.sub}</div>
      </div>
      <div style={{ display: 'flex', gap: 4, marginTop: 15 }}>
        {insights.map((_, i) => <div key={i} style={{ width: i === index ? 12 : 4, height: 4, borderRadius: 2, background: i === index ? C.teal : C.grayDim, transition: '0.3s' }} />)}
      </div>
    </Card>
  );
}
...
function RandomShow({ concerts }) {
  ...
  const spin = () => {
    ...
    const interval = setInterval(() => {
      ...
      iterations++;
      if (iterations >= 12) { clearInterval(interval); setSpinning(false); }
    }, 70);
  };
  ...
  return (
    <Card neon glow={!spinning} style={{ minHeight: 150, border: `1px solid ${spinning ? C.grayDim : C.purple + '66'}`, display: 'flex', flexDirection: 'column', justifyContent: 'center', transition: '0.3s' }}>
      ...
      <div style={{ opacity: spinning ? 0.3 : 1, transition: '0.2s' }}>
        <div style={{ fontFamily: "'Bebas Neue'", fontSize: '2.2rem', color: C.white, lineHeight: 1, marginBottom: 4, textShadow: !spinning ? `0 0 10px ${C.purple}33` : 'none' }}>{artistName}</div>
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 20 }}>
      <svg width={140} height={140} viewBox="0 0 140 140">
        ...
        <circle cx={cx} cy={cy} r={r} fill="none" stroke={C.teal} strokeWidth={14} strokeDasharray={`${festDash} ${circ}`} strokeDashoffset={0} strokeLinecap="round" transform={`rotate(-90 ${cx} ${cy})`} style={{ filter: `drop-shadow(0 0 4px ${C.teal}66)` }} />
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
      {top3.map(([name, count], i) => (
        <div key={name} style={{ display: 'flex', alignItems: 'center', gap: 12, background: `${colors[i]}0a`, border: `1px solid ${colors[i]}33`, borderLeft: `3px solid ${colors[i]}`, borderRadius: 4, padding: '10px 14px' }}>
          ...
          <div style={{ flex: 1 }}><div style={{ fontFamily: "'Bebas Neue'", fontSize: '1rem', letterSpacing: '0.06em', color: C.white }}>{name}</div></div>
      ...
      {top3.length === 0 && <div style={{ color: C.gray, fontSize: '0.8rem' }}>No festival data yet.</div>}
    </div>
  );
}
...
function DecadeBlocks({ sets }) {
  ...
  const max    = Math.max(...Object.values(dec), 1);
      ...
      {Object.entries(dec).map(([decade, count], i) => (
        <div key={decade} style={{ background: `${colors[i]}18`, border: `1px solid ${colors[i]}44`, borderBottom: `3px solid ${colors[i]}`, borderRadius: 4, padding: '10px 8px', textAlign: 'center' }}>
          ...
          <div style={{ marginTop: 6, height: 2, background: C.border, borderRadius: 1, overflow: 'hidden' }}>
            <div style={{ height: '100%', width: `${(count / max) * 100}%`, background: colors[i], borderRadius: 1 }} />
          </div>
        </div>
      ))}
    </div>
  ...
  return (
    <div ref={topRef} style={{ padding: '24px 0' }} className="fade-in">
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: C.gray, marginBottom: 16, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
        Artists seen {HALL_OF_FAME_MIN}+ times — click any to expand
      </div>
      ...
      {selectedData && (
        <div className="fade-in" style={{ background: C.bgCard, border: `1px solid ${C.teal}55`, borderRadius: 8, padding: '18px 20px', marginBottom: 24, boxShadow: `0 0 20px ${C.teal}22` }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {onShare && (
                <button onClick={() => onShare(selectedData.artist, selectedData.shows)} style={{ fontFamily: "'Space Mono'", fontSize: 9, background: C.tealFaint, border: `1px solid ${C.borderLit}`, color: C.teal, borderRadius: 4, padding: '4px 10px', cursor: 'pointer' }}>📤 Share</button>
              )}
            ...
            {[...selectedData.shows].reverse().map((s, i) => {
              ...
              return (
                <div key={i} style={{ position: 'relative', marginBottom: 12, paddingLeft: 14 }}>
                  <div style={{ position: 'absolute', left: -7, top: 4, width: 8, height: 8, borderRadius: '50%', background: s.is_festival ? C.teal : (hasSet ? C.gold : C.grayDim), border: `1px solid ${s.is_festival ? C.teal : (hasSet ? C.gold : C.border)}`, boxShadow: s.is_festival ? `0 0 8px ${C.teal}aa` : (hasSet ? `0 0 8px ${C.gold}aa` : 'none'), zIndex: 2 }} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px,1fr))', gap: 10 }}>
        ...
        {artists.length === 0 && <div style={{ color: C.gray, gridColumn: '1/-1', textAlign: 'center', padding: 60 }}>See {HALL_OF_FAME_MIN}+ shows to unlock.</div>}
      </div>
    </div>
  );
}
            ...
            <div key={ci} style={{ borderRight: ci < columns.length - 1 ? `1px solid ${C.border}` : 'none' }}>
              ...
              <div style={{ padding: '8px 10px', display: 'flex', flexDirection: 'column', gap: 5 }}>
                {stageBands.map((band, bi) => (
                  <div key={bi} style={{ background: C.bgCardAlt, borderRadius: 4, padding: '6px 8px', borderLeft: `2px solid ${STAGE_COLORS[ci % STAGE_COLORS.length]}`, fontSize: '0.75rem', color: C.white, lineHeight: 1.3 }}>{band}</div>
                ))}
              </div>
            </div>
          ))}
...
// ─── DAY CARD ─────────────────────────────────────────────────────────────────
function DayCard({ event, onEdit }) {
  const bands = event.bands || [];
  const hasSetlist = event.has_setlist || (event.has_setlist_names && event.has_setlist_names.trim() !== '');
  return (
    <div className="day-card-hover" data-date={event.date} style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 8, marginBottom: 8, overflow: 'hidden' }} onClick={() => onEdit && onEdit(event)}>
      ...
      <div style={{ padding: '8px 16px 12px', display: 'flex', flexWrap: 'wrap', gap: 5 }}>
        {bands.map((b, i) => <span key={i} style={{ fontSize: '0.78rem', color: C.white, background: C.bgCardAlt, border: `1px solid ${C.border}`, borderRadius: 4, padding: '4px 9px' }}>{b}</span>)}
      </div>
    </div>
  );
}
    ...
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} onClick={onTeleport}
      ...
      <div style={{ width: '43%', padding: 20, borderRadius: 12, background: hovered ? hexToRgba(themeColor, 0.15) : hexToRgba(themeColor, 0.05), border: `1px solid ${hovered ? themeColor : hexToRgba(themeColor, 0.3)}`, borderLeft: isLeft ? `6px solid ${themeColor}` : `1px solid ${hovered ? themeColor : hexToRgba(themeColor, 0.3)}`, borderRight: !isLeft ? `6px solid ${themeColor}` : `1px solid ${hovered ? themeColor : hexToRgba(themeColor, 0.3)}`, transform: hovered ? 'scale(1.03) translateY(-5px)' : 'scale(1)', transition: 'all 0.4s cubic-bezier(0.175,0.885,0.32,1.275)', boxShadow: hovered ? `0 15px 40px -15px ${themeColor}66` : 'none', zIndex: hovered ? 20 : 1 }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 15 }}>
          {bands.map((band, idx) => (
            <span key={idx} style={{ fontFamily: idx === 0 ? "'Bebas Neue'" : "'Space Mono'", fontSize: idx === 0 ? '2rem' : '0.75rem', color: '#fff', lineHeight: 1, opacity: idx !== 0 && !hovered ? 0.4 : 1 }}>
              {band}{idx < bands.length - 1 && idx !== 0 ? ' •' : ''}
            </span>
          ))}
        </div>
        <div style={{ marginTop: 10, paddingTop: 10, borderTop: `1px solid ${hexToRgba(themeColor, 0.2)}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span style={{ fontFamily: "'Space Mono'", fontSize: 9, color: '#fff', opacity: 0.7 }}>{item.venue?.toUpperCase()} // {item.city?.toUpperCase()}</span>
          <span style={{ fontFamily: "'Space Mono'", fontSize: 8, color: themeColor, fontWeight: 'bold' }}>{genre.toUpperCase()}</span>
        </div>
      </div>
    </div>
  );
    ...
    return Object.entries(groups).sort((a, b) => b[0] - a[0]).map(([year, yearShows]) => {
      ...
      yearShows.forEach((show, idx) => {
        ...
        if (nextShow) {
          ...
          gap = Math.ceil(Math.abs(d1 - d2) / 86400000);
        }
        showCounter++;
        ...
        {yearsData.map(([year, flow], yIdx) => (
          <div key={year} style={{ position: 'relative', marginBottom: 120 }}>
            <div style={{ position: 'absolute', left: '-180px', top: 0, bottom: 0, width: '100px', zIndex: 1 }}>
              <div style={{ position: 'sticky', top: '250px', fontFamily: "'Bebas Neue'", fontSize: '6.5rem', color: 'transparent', WebkitTextStroke: `2px ${yIdx % 2 === 0 ? '#00f2ff' : '#9d00ff'}`, opacity: 0.6, transform: 'rotate(-90deg)', transformOrigin: 'center' }}>{year}</div>
            </div>
            ...
            <div style={{ width: '100%', padding: '0 20px' }}>
              {flow.map((item) => item.type === 'MONTH_MARKER'
                ? (
                  <div key={item.id} style={{ margin: '80px 0 40px', textAlign: 'center', position: 'relative', zIndex: 10 }}>
                    <span style={{ fontFamily: "'Space Mono'", fontSize: 14, color: '#fff', background: '#0a0a0c', padding: '8px 24px', borderRadius: 4, border: '2px solid #9d00ff', fontWeight: 700, boxShadow: '0 0 20px rgba(157,0,255,0.3)', letterSpacing: 6 }}>{item.label}</span>
                  </div>
                ) : (
                  <TimelineCard key={item.id} item={item} isLeft={item.side === 'left'} marginTop={item.gapDays <= 2 ? 20 : Math.min(item.gapDays * 2, 150)} onTeleport={() => teleport(item.date)} />
                )
              )}
            </div>
          </div>
...
function ByDayTab({ dayGroups, onEdit }) {
  return (
    <div style={{ padding: '24px 0' }} className="fade-in">
      {dayGroups.map(event =>
        event.is_festival
          ? <FestivalScheduleCard key={event.id} event={event} />
          : <DayCard key={event.id} event={event} onEdit={onEdit} />
      )}
      {dayGroups.length === 0 && <div style={{ textAlign: 'center', color: C.gray, padding: 60 }}>No shows match your filters.</div>}
    </div>
  );
}
  ...
  return (
    <div style={{ marginTop: 20 }} className="fade-in">
      {festGroupings.map(fest => (
        <div key={fest.name} style={{ marginBottom: 36 }}>
          <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.8rem', color: C.teal, letterSpacing: '0.08em', borderBottom: `1px solid ${C.teal}33`, paddingBottom: 6, marginBottom: 14 }}>{fest.name}</div>
          {Object.entries(fest.years).sort((a, b) => b[0].localeCompare(a[0])).map(([year, shows]) => (
            <div key={year} style={{ paddingLeft: 20, marginBottom: 12 }}>
              <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.1rem', color: C.gold, marginBottom: 6 }}>{year}</div>
              {shows.map(s => <FestivalScheduleCard key={s.id} event={s} compact />)}
            </div>
          ))}
        </div>
      ))}
      {festGroupings.length === 0 && <div style={{ textAlign: 'center', color: C.gray, padding: 60 }}>No festival data yet.</div>}
    </div>
  );
}
...
function PassportTab({ passport }) {
  return (
    <div style={{ padding: '24px 0' }} className="fade-in">
      <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 10, color: C.gray, marginBottom: 20, letterSpacing: '0.1em', textTransform: 'uppercase' }}>
        Your festival attendance record — every fest, every year
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px,1fr))', gap: 14 }}>
        {passport.map(f => (
          <Card key={f.name} neon>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.3rem', color: C.gold, letterSpacing: '0.06em', lineHeight: 1 }}>{f.name}</div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.6rem', color: C.teal, lineHeight: 1 }}>{f.days}</div>
                <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 7, color: C.gray, textTransform: 'uppercase' }}>days</div>
              </div>
            </div>
            <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, color: C.gray, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
              {f.years.length} {f.years.length === 1 ? 'year' : 'years'} attended
            </div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
              {f.years.map(y => <span key={y} style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, background: `${C.gold}22`, color: C.gold, border: `1px solid ${C.gold}44`, padding: '2px 6px', borderRadius: 3 }}>{y}</span>)}
            </div>
          </Card>
        ))}
        {passport.length === 0 && <div style={{ color: C.gray, textAlign: 'center', gridColumn: '1/-1', padding: 60 }}>No festival passport stamps yet.</div>}
      </div>
    </div>
  );
}

function BrowseTab({ browseView, setBrowseView, search, setSearch, yearFilter, setYearFilter, festFilter, setFestFilter, sortCol, setSortCol, sortDir, setSortDir, paged, page, setPage, totalPages, artistRows, years, onShare, onEdit }) {
  return (
    <div style={{ marginTop: 20 }} className="fade-in">
      {/* Filter bar */}
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, marginBottom: 20, background: C.bgCard, padding: 15, borderRadius: 8, border: `1px solid ${C.border}` }}>
        ...
        <select value={yearFilter} onChange={e => setYearFilter(e.target.value)} style={{ ...inputSt, minWidth: 100 }}>
          <option value="all">All Years</option>
          {years.map(y => <option key={y} value={y}>{y}</option>)}
        </select>
        <select value={festFilter} onChange={e => setFestFilter(e.target.value)} style={{ ...inputSt, minWidth: 130 }}>
        ...
        <div style={{ display: 'flex', background: C.bgCardAlt, borderRadius: 4, padding: 2, border: `1px solid ${C.border}` }}>
          {['shows','artists'].map(v => (
            <button key={v} onClick={() => setBrowseView(v)} style={{ padding: '6px 14px', fontSize: 10, fontFamily: "'Space Mono'", letterSpacing: '0.1em', textTransform: 'uppercase', background: browseView === v ? C.teal : 'transparent', color: browseView === v ? C.bg : C.gray, border: 'none', cursor: 'pointer', borderRadius: 3, transition: '0.15s' }}>{v}</button>
          ))}
        </div>
      </div>
          ...
          <div style={{ background: C.bgCard, border: `1px solid ${C.border}`, borderRadius: 8, overflow: 'hidden' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '0.78rem' }}>
              <thead>
                <tr style={{ background: C.bgCardAlt }}>
                  {[['date','Date'],['artist','Artist'],['venue','Venue'],['city','City']].map(([col, label]) => (
                    <th key={col} onClick={() => { setSortCol(col); setSortDir(d => d === 'asc' ? 'desc' : 'asc'); }}
                      style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, letterSpacing: '0.15em', textTransform: 'uppercase', padding: '10px 12px', textAlign: 'left', color: sortCol === col ? C.teal : C.tealDim, borderBottom: `1px solid ${C.border}`, cursor: 'pointer', userSelect: 'none' }}>
              ...
              <tbody>
                ...
                {paged.length === 0 && (
                  <tr><td colSpan={5} style={{ textAlign: 'center', color: C.gray, padding: 40 }}>No results.</td></tr>
                )}
              </tbody>
            </table>
          </div>
      ...
      {browseView === 'artists' && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px,1fr))', gap: 12 }}>
          {artistRows.map(row => (
            <Card key={row.artist} neon onClick={() => onShare(row.artist, row.shows)} style={{ cursor: 'pointer' }}>
              <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.teal, marginBottom: 4 }}>{row.artist}</div>
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 8, color: C.gray, textTransform: 'uppercase', letterSpacing: '0.1em' }}>{row.shows.length} shows</div>
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: 7, color: C.grayDim, marginTop: 4 }}>
                {[...new Set(row.shows.map(s => getYear(s.date)).filter(Boolean))].sort().join(' · ')}
              </div>
            </Card>
          ))}
          {artistRows.length === 0 && <div style={{ color: C.gray, textAlign: 'center', gridColumn: '1/-1', padding: 40 }}>No artists match your filters.</div>}
        </div>
      )}
    </div>
  );
      ...
      {totalPages > 1 && (
        <div style={{ display: 'flex', justifyContent: 'center', gap: 5, marginTop: 14 }}>
          ...
          {Array.from({ length: Math.min(7, totalPages) }, (_, i) => {
            ...
            return <Btn key={p} variant={p === page ? 'primary' : 'secondary'} onClick={() => setPage(p)} style={{ padding: '5px 10px' }}>{p}</Btn>;
          })}
...
// ─── MAIN APP (single export, defined once) ───────────────────────────────────
export default function App() {
  ...
  const [newUpcoming, setNewUpcoming] = useState({ artist: '', venue: '', date: '', status: 'TICKETS BOUGHT' });
  ...
  // ── MASTER DATA (each useMemo defined exactly once) ────────────────────────

  /** THE master array: flattens multi-band concerts into individual set rows */
  ...
  const years = useMemo(() =>
    [...new Set(concerts.map(c => getYear(c.date)).filter(Boolean))].sort()
  , [concerts]);
  ...
  const dashboardStats = useMemo(() => {
    ...
    // Fresh Blood: first seen in 2025 or later
    ...
    return {
      ...
      newDiscoveries: [...recentBands].filter(b => !oldBands.has(b)).length,
    };
  }, [concerts, allSetsList]);
  ...
  const genreStats = useMemo(() => {
    const counts = {};
    concerts.forEach(c => {
      const artist = Array.isArray(c.bands) ? c.bands[0] : c.artist;
  ...
  // ── FILTER ENGINE (single definition) ──────────────────────────────────────
  ...
  /** Filtered + sorted sets for Browse > Shows table */
  ...
  /** Artist rollup for Browse > Artists grid */
  const artistRows = useMemo(() => {
    if (browseView !== 'artists') return [];
    const m = {};
    applyFilters(allSetsList, true).forEach(s => {
      if (!m[s.artist]) m[s.artist] = { artist: s.artist, shows: [] };
      m[s.artist].shows.push(s);
    });
  ...
  /** Flat filtered concert list for By Day tab */
  const dayGroups = useMemo(() =>
    applyFilters(concerts).sort((a, b) => (b.date || '').localeCompare(a.date || ''))
  ...
  const deleteUpcomingShow = async (id) => {
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', paddingBottom: 60 }}>
      ...
      {/* ── HEADER ── */}
      <div style={{ background: `linear-gradient(180deg,#050508 0%,${C.bgCard} 100%)`, borderBottom: `2px solid ${C.teal}33`, padding: '45px 24px 35px', textAlign: 'center' }}>
        ...
        <div style={{ marginTop: 18, fontFamily: "'Space Mono'", fontSize: '0.75rem', color: C.gray, display: 'flex', justifyContent: 'center', gap: 15, flexWrap: 'wrap' }}>
      ...
      <nav style={{ background: C.bgCard, borderBottom: `1px solid ${C.teal}33`, display: 'flex', overflowX: 'auto', position: 'sticky', top: 0, zIndex: 200 }}>
        {TABS.map(([id, label]) => (
          <button key={id} onClick={() => setActiveTab(id)}
            style={{ fontFamily: "'Space Mono'", fontSize: 10, color: activeTab === id ? C.teal : C.gray, background: 'none', border: 'none', borderBottom: activeTab === id ? `2px solid ${C.teal}` : '2px solid transparent', padding: '12px 16px', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
            {label}
          </button>
        ))}
      </nav>
      ...
      <main style={{ maxWidth: 1200, margin: '0 auto', padding: '0 20px' }}>
        ...
        {activeTab === 'dashboard' && (
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(180px,1fr))', gap: 12, marginBottom: 20, marginTop: 20 }}>
              <Card neon>
                <div style={{ fontSize: 8, color: C.tealDim, fontFamily: "'Space Mono'" }}>HEAVY ROTATION</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.white }}>{dashboardStats.topBand}</div>
                <div style={{ fontSize: 7, color: C.gray, fontFamily: "'Space Mono'" }}>SEEN {dashboardStats.topCount} TIMES</div>
              </Card>
              <Card neon>
                <div style={{ fontSize: 8, color: C.tealDim, fontFamily: "'Space Mono'" }}>TRAVELER</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.white }}>{dashboardStats.stateCount} STATES</div>
              </Card>
              <Card neon>
                <div style={{ fontSize: 8, color: C.tealDim, fontFamily: "'Space Mono'" }}>VENUE MASTERY</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.white }}>{dashboardStats.venueCount} STAGES</div>
              </Card>
              <Card neon>
                <div style={{ fontSize: 8, color: C.tealDim, fontFamily: "'Space Mono'" }}>FRESH BLOOD</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.white }}>{dashboardStats.newDiscoveries} NEW ACTS</div>
                <div style={{ fontSize: 7, color: C.gray, fontFamily: "'Space Mono'" }}>FIRST SEEN IN '25/'26</div>
              </Card>
              <Card neon>
                <div style={{ fontSize: 8, color: C.tealDim, fontFamily: "'Space Mono'" }}>TOTAL VOLUME</div>
                <div style={{ fontFamily: "'Bebas Neue'", fontSize: '1.2rem', color: C.white }}>{dashboardStats.totalSets} SETS</div>
              </Card>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr 1fr', gap: 15, marginBottom: 20 }}>
              ...
              <div style={{ background: '#0a0a0a', border: '3px solid #222', borderRadius: 12, overflow: 'hidden', boxShadow: '0 0 30px rgba(255,204,0,0.15)' }}>
                <div style={{ background: '#ffcc00', color: '#000', padding: '4px 0', overflow: 'hidden', borderBottom: '2px solid #000' }}>
                  <div className="marquee-text" style={{ fontFamily: "'Space Mono'", fontSize: 10, fontWeight: '900', whiteSpace: 'nowrap', letterSpacing: 1 }}>
                    FOR YOUR CONSIDERATION • STAGING THE VIBE • TICKETS SECURED? • TOUR BUS INBOUND • FOR YOUR CONSIDERATION • STAGING THE VIBE • TICKETS SECURED? • TOUR BUS INBOUND •&nbsp;
                  </div>
                </div>
                <div style={{ padding: 20 }}>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 15 }}>
                    <input placeholder="Artist *" value={newUpcoming.artist} onChange={e => setNewUpcoming({ ...newUpcoming, artist: e.target.value })} style={{ background: '#111', border: '1px solid #333', color: '#ffcc00', fontSize: 10, padding: 8, flex: '2 1 120px', borderRadius: 4 }} />
                    <input placeholder="Venue" value={newUpcoming.venue} onChange={e => setNewUpcoming({ ...newUpcoming, venue: e.target.value })} style={{ background: '#111', border: '1px solid #333', color: '#fff', fontSize: 10, padding: 8, flex: '1 1 100px', borderRadius: 4 }} />
                    <input type="date" value={newUpcoming.date} onChange={e => setNewUpcoming({ ...newUpcoming, date: e.target.value })} style={{ background: '#111', border: '1px solid #333', color: '#fff', fontSize: 10, padding: 7, flex: '1 1 110px', borderRadius: 4, colorScheme: 'dark' }} />
                    <select value={newUpcoming.status} onChange={e => setNewUpcoming({ ...newUpcoming, status: e.target.value })} style={{ background: '#111', border: '1px solid #333', color: '#ffcc00', fontSize: 10, padding: 8, borderRadius: 4 }}>
                  ...
                  <div style={{ maxHeight: 140, overflowY: 'auto' }}>
                    {upcoming.map((show, i) => (
                      <div key={show.id || i} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10, paddingBottom: 8, borderBottom: '1px solid #1a1a1a' }}>
                        <div>
                          <div className="marquee-letter" style={{ fontSize: '1.1rem', lineHeight: 1 }}>{show.artist}</div>
                          <div style={{ color: '#666', fontSize: 8 }}>@{show.venue || 'TBA'}</div>
                        </div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <div style={{ color: '#fff', fontSize: 10 }}>{show.date}</div>
                          <button onClick={() => deleteUpcomingShow(show.id)} style={{ background: 'none', border: 'none', color: '#444', cursor: 'pointer', fontSize: 14 }}>✕</button>
                        </div>
                      </div>
                    ))}
                    {upcoming.length === 0 && <div style={{ color: '#333', fontFamily: "'Space Mono'", fontSize: 9, textAlign: 'center', padding: 20 }}>NO SHOWS QUEUED</div>}
                  </div>
                </div>
              </div>
            ...
            {/* Most Seen + Setlist Spotlight */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
              <Card neon>
                <CardTitle>Most Seen Artists</CardTitle>
                {artistCounts.slice(0, 6).map(a => (
                  <div key={a.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: `1px solid ${C.border}` }}>
                    <span style={{ color: C.white, fontSize: '0.85rem' }}>{a.name}</span>
              ...
              <Card neon>
                ...
                <SetlistSpotlight concerts={concerts} onVault={() => setActiveTab('hof')} />
              </Card>
            </div>
          </>
        )}
        ...
        {activeTab === 'byDay' && (
          <>
            <div style={{ display: 'flex', gap: 10, marginTop: 20, marginBottom: 16, flexWrap: 'wrap' }}>
              ...
              <select value={festFilter} onChange={e => setFestFilter(e.target.value)} style={{ ...inputSt, minWidth: 140 }}>
                <option value="all">All Types</option>
                <option value="fest">Festival Only</option>
                <option value="solo">Standalone Only</option>
              </select>
            </div>
            <ByDayTab dayGroups={dayGroups} onEdit={setEditTarget} />
          </>
        )}
        ...
        {/* ════ HALL OF FAME ════ */}
        {activeTab === 'hof' && (
          <HallOfFame sets={allSetsList} onShare={(a, s) => setShareCard({ artist: a, shows: s })} />
        )}
